...
function sendMessage() {
  ...
  Object.values(cartItems).map((product) => {
    formatedProductList.push({
      name: product.name,
      quantity: product.inCart,
    });
  });

  console.log(formatedProductList);
  ...
  checkProductListSize = () => {
    if (formatedProductList.length > 1) {
      ...
      formatedProductList.forEach((product) => {
        message += `${product.name}, Quantidade: ${product.quantity}\n`;
      });
    } else {
      message = `Olá, estou interessado em comprar esse produto: `;
      formatedProductList.forEach((product) => {
        message += `${product.name}, Quantidade: ${product.quantity}\n`;
      });
    }
    ...
    Object.values(cartItems).map((item) => {
      productContainer.innerHTML += `
            <tr id = 'product_${item.tag}'>
                ...
                <td>
                    <input onchange="changeCartNumber('${
                      item.tag
                    }')" class="numbers" type="number" value="${item.inCart}">
                </td>
...
function removeItems(productTag) {
  ...
  localStorage.setItem(
    "cartNumbers",
    parseInt(numbersInCart) - parseInt(totalInCart)
  );
  localStorage.setItem("productsInCart", JSON.stringify(cartItems));
...
function changeCartNumber(productTag) {
  ...
  let rawNumber = rawString
    .replace(/([R$,])/g, "")
    .slice(0, rawString.length - 5);
  let productPrice = 0;
  let productArray = [];
  let productList = {};
  ...
  Object.values(cartItems).forEach((product) => {
    if (product.tag == productTag) {
      ...
      if (product.inCart < element.children[4].lastElementChild.valueAsNumber) {
        ...
        localStorage.setItem(
          "totalCoast",
          parseInt(totalCoasts) + productPrice
        );
        localStorage.setItem("productsInCart", JSON.stringify(cartItems));
        ...
        product.inCart > element.children[4].lastElementChild.valueAsNumber &&
        element.children[4].lastElementChild.valueAsNumber !== 0
      ) {
        productPrice = product.price;
        product.inCart -= 1;
